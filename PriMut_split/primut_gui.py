...
class InputPage(ctk.CTkFrame):
    ...
    def _run_workflow(self):
        ...
        try:
            ...
            pg.validate_mutations_against_sequence(seq, parsed)
            primer_data = pg.generate_primers_for_construct(seq, parsed)
...
class MutationFailureHandler(ctk.CTkFrame):
    ...
    def _setup_ui(self):
        ...
        cf.grid(row=1, column=0, columnspan=2, sticky="nsew",
                padx=10, pady=5)
        cf.grid_columnconfigure(0, weight=2)
        cf.grid_columnconfigure(1, weight=1)
        cf.grid_rowconfigure(0, weight=1)

        # Variant table (left)
        self._create_variant_table(cf)
        # Search (right)
        self._create_search_section(cf)
        ...
        for row, items in [
            (0, [("Validate Selection", self.validate_selection,
                  {"fg_color": "orange", "width": 150}),
                 ("Process Failed Mutations", self.process_failures,
                  {"fg_color": "green", "width": 200}),
                 ("Clear All Checks", self._clear_all_checks,
                  {"width": 120})]),
            (1, [("Undo Last Processing", self._undo_last_processing,
                  {"fg_color": "gray", "width": 150}),
                 ("Refresh Variants", self.load_variants,
                  {"width": 120})]),
        ]:
            for col, (txt, cmd, kw) in enumerate(items):
                ctk.CTkButton(bf, text=txt, command=cmd,
                              font=self.controller.MEDIUMFONT, **kw
                              ).grid(row=row, column=col,
                                     padx=10, pady=5, sticky="w")

        self.status_label = ctk.CTkLabel(bf, text="",
                                         font=self.controller.MEDIUMFONT)
        self.status_label.grid(row=2, column=0, columnspan=3,
                               sticky="w", padx=10, pady=5)

    def _create_variant_table(self, parent):
        ...
        hdr = ctk.CTkFrame(vf)
        hdr.grid(row=0, column=0, sticky="ew", padx=5, pady=5)
        hdr.grid_columnconfigure(1, weight=1)
        for col, txt in [(0, "Variant"), (1, "Mutations"),
                         (2, "Present"), (3, "Failed")]:
            ctk.CTkLabel(hdr, text=txt,
                         font=self.controller.MEDIUMFONT
                         ).grid(row=0, column=col, padx=10, pady=5, sticky="w")

        self.variant_scroll = ScrollableFrameWithWheel(vf)
        self.variant_scroll.grid(row=1, column=0, sticky="nsew",
                                 padx=5, pady=5)
        self.variant_scroll.grid_columnconfigure(1, weight=1)
    ...
    def _update_variant_display(self):
        ...
        filtered = len(self.filtered_variants)
        self.results_label.configure(
            text=(f"Showing all {total} variants"
                  if not any(v.get().strip() for v in self.search_vars)
                  else f"Showing {filtered} of {total}"))

        for i, (vid, mut_str) in enumerate(
            sorted(self.filtered_variants.items(),
                   key=lambda x: natural_sort_key(x[0]))
        ):
            muts = ([m.strip() for m in mut_str.split(',') if m.strip()]
                    if mut_str and mut_str != "(none)" else [])
            ctk.CTkLabel(self.variant_scroll, text=f"{vid}:",
                         font=self.controller.MEDIUMFONT
                         ).grid(row=i, column=0, sticky="w",
                                padx=(10, 5), pady=5)
            ctk.CTkLabel(self.variant_scroll, text=mut_str or "(none)",
                         font=self.controller.SMALLFONT
                         ).grid(row=i, column=1, sticky="w",
                                padx=5, pady=5)

            check_vars: list = []
            cf = ctk.CTkFrame(self.variant_scroll)
            cf.grid(row=i, column=2, sticky="w", padx=5, pady=2)
            ff_frame = ctk.CTkFrame(self.variant_scroll)
            ff_frame.grid(row=i, column=3, sticky="w", padx=5, pady=2)

            for m in muts:
                pv = ctk.IntVar(value=1)  # present by default
                fv = ctk.IntVar(value=0)  # not failed by default
                ctk.CTkCheckBox(cf,  text=m, variable=pv,
                                font=self.controller.SMALLFONT
                                ).pack(anchor="w")
                ctk.CTkCheckBox(ff_frame, text=m, variable=fv,
                                font=self.controller.SMALLFONT
                                ).pack(anchor="w")
                check_vars.append((m, pv, fv))

            self.variant_rows[vid] = {
                'mutations':  muts,
                'checkboxes': check_vars,
            }

    def get_failed_mutations(self) -> Dict:
        result: Dict = {}
        for vid, data in self.variant_rows.items():
            failed  = [m for m, pv, fv in data['checkboxes'] if fv.get()]
            present = [m for m, pv, fv in data['checkboxes'] if pv.get()]
            if failed:
                result[vid] = {
    ...
    def validate_selection(self):
        """Run hierarchy check in background thread."""
        ...
        marked    = {vid: set(info['failed'])
                     for vid, info in failed.items()}

        validate_btn = None  # captured below; re-enabled in callback

        def _task():
            warnings: List[str] = []
            variant_sets = {vid: frozenset(
                m.strip() for m in muts.split(',') if m.strip()
            ...
            for mv, failed_muts in marked.items():
                ...
                if unchecked:
                    ...
                    for pid, pmuts in unchecked:
                        txt += f"      • {pid}: check {', '.join(sorted(pmuts))}\n"
                    warnings.append(txt)
        ...
        def _show(warnings):
            if warnings:
                ...
                tb.insert("1.0", "⚠️ VALIDATION WARNINGS ⚠️\n" +
                          "=" * 60 + "\n" + "".join(warnings))
                tb.configure(state="disabled")
    ...
    def process_failures(self):
        ...
        if not messagebox.askyesno(
            ...
            "2. Create repair variants\n"
            "3. Generate repair protocol PDF\n\nContinue?"
        ):
            return

        try:
            self.status_label.configure(
                text="Processing...", text_color="blue")
            db          = self.controller.get_databank()
            ...
            self._generate_repair_protocol(repair_variants, updated_variants,
                                           variant_prefix)
    ...
    def _undo_last_processing(self):
        ...
        try:
            ...
            self.status_label.configure(
                text="Undo successful", text_color="green")
        except Exception as e:
            messagebox.showerror("Undo Error", f"Failed: {e}")
